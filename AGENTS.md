...
- **The Docker Hub overview is repository metadata, pushed separately.** The OCI labels GoReleaser stamps on the manifest never reach the listing page, so `hub.docker.com/r/kestra/kestractl` renders blank unless something calls the Docker Hub API. `release.yml`'s `Sync the Docker Hub description` step does that from `docs/docker-hub.md` — an image-focused page, not `README.md`, which is well past Docker Hub's 25,000-character `full_description` cap and whose relative links would all 404 there. It runs on stable releases only (`steps.rolling.outputs.stable`): the page describes the tags a plain `docker pull` resolves to, and a prerelease moves neither `:latest` nor `:X.Y`. It also needs a Docker Hub **password or a write-scoped PAT** — a read-only token pushes images fine and then 401s on this one API. GHCR needs no equivalent: it reads its description from the `org.opencontainers.image.description` label already set in `.goreleaser.yml`.
