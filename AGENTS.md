...
- **`main`** — kestractl v3, targeting Kestra 2.x (Go SDK `go-sdk/v2`). Tags: `v3.x.y`. kestractl's own major version is independent of Kestra's — `main` went from `v2.x.y` to `v3.x.y` on a breaking `kestractl` change without any change to which Kestra versions it targets.
...
**`main` is GA, so the table above applies literally.** The base tag is whatever `next-version.sh` finds via `git describe` — a `feat:` bumps its minor, a breaking change its major. `v2.0.0` was the last deliberate manual tag push, and the line has since auto-bumped past a `!:` commit straight through to `v3.0.0` and onward on ordinary `feat:`/`fix:` commits. The rc line is closed.
...
Each auto-tagged release is what a default `curl … | bash` install resolves to, because `install.sh`'s `DEFAULT_MAJOR` picks the newest release of that major line **in creation order, prereleases included**. `DEFAULT_MAJOR` must track whatever major line `main` is currently releasing (`3` today) — it does not update itself when a breaking change bumps the major version, so a `<type>!:` release is also a reminder to bump it. So publishing a prerelease on the current major line would hand it to every default install — a reason not to reopen an rc line on `main` casually. Either way, that is why the release gate is the full `Tests` suite (unit, installer smoke, e2e matrix) and not a fast subset.

**GitHub's "latest" is a badge, not the install default.** GitHub has one repo-wide "latest" release, and `.goreleaser.yml` sets `release.make_latest: "true"` on `main` since it's the only GA line. `releases/v1` still carries `"true"` in its own config, which is harmless: the flag only takes effect when GoReleaser publishes a release, and that branch is legacy and not released any more. Since `install.sh` resolves the newest release of a major line itself (`DEFAULT_MAJOR=3`, with `VERSION=1` for the legacy line), that pointer does not decide what a default install gets — it drives the GitHub UI badge, and the in-CLI update notifier, which reads `/releases/latest`. `/releases/latest` therefore points at the current major line (3.x), so **v1 users are notified about a major upgrade**. Note GitHub cannot mark a draft or prerelease as latest, so `make_latest: "true"` is inert on any prerelease `main` might publish.
...
Six things to know before touching this:
...
- **The Docker Hub overview is repository metadata, maintained by hand.** The OCI labels GoReleaser stamps on the manifest never reach the listing page, so `hub.docker.com/r/kestra/kestractl` shows whatever was last typed into Docker Hub's own settings and nothing else. `docs/docker-hub.md` is the source of that text: edit it here, then paste it into the repository's *Overview* field. Nothing in CI syncs the two, so **assume the published page is stale** and re-paste when the file changes. This is how the rest of the org works — `kestra/kestra`'s overview was pasted once in September 2025 and has not tracked its README since. Automating it was tried on this repo and reverted: the Docker Hub API rejects the org's registry credentials with a bare `Forbidden`, because pushing an image and editing repository metadata are different scopes, and no org secret currently holds the latter.
